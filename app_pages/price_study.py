...
def correlation_study():
    ...
    st.info(
        ...
        f"* When a house has a garage, the price of the house increases, depending on the size of the garage.\n"
        f"* The higher the overall quality of the house, the more expensive it is.\n"
        f"* House prices is higher when the house is newer.\n"
    )
    ...
    feature_x = st.selectbox(
        label='All features with area is measured in square feet (sq ft)', options=vars_to_study)

    fig = px.scatter(df, x=feature_x, y=df_eda['SalePrice']).update_layout(
